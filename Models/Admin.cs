...
namespace HiTech.Models
{
    public class Admin
    {
        ...
        {
            SqlCommand cmd = new SqlCommand("insert into [dbo].[Register] (name,email,password,status)values('" + name + "','" + email + "','" + password + "','" + status + "')", con);
            con.Open();
            return cmd.ExecuteNonQuery();

        }
        public int productInsert(string name, int id, string brand, string color, string conditin, string des, string bid, string price, string start, string end)
        {
            SqlCommand cmd = new SqlCommand("insert into [dbo].[Table_1] (product_name,user_id,brand,color,condition,description,starting_bid,price,start_time,end_time,status)values('" + name + "','" + id + "','" + brand + "','" + color + "','" + conditin + "','" + des + "','" + bid + "','" + price + "','" + start + "','" + end + "','" + false + "')", con);
            con.Open();
            return cmd.ExecuteNonQuery();
